#!/usr/bin/env python

'''
# Team ID:          < 158 >
# Theme:            < vitarana_drone>
# Author List:      < shoaib razil,sidharth,vishnu,muhammed sinan k>
# Filename:         < attitude_controller.py >
# Functions:        < [landing_callback, imu_callback, drone_cmd_callback, pid] >
# Global variables: < PI >
'''

# Importing the required libraries

from vitarana_drone.msg import *
from pid_tune.msg import PidTune
from sensor_msgs.msg import Imu
from std_msgs.msg import Float64
import rospy
import time
import tf
import math

PI=math.pi

class Edrone():
    """docstring for Edrone"""
    def __init__(self):
        rospy.init_node('attitude_controller')  # initializing ros node with name drone_control

        # This corresponds to your current orientation of eDrone in quaternion format. This value must be updated each time in your imu callback
        # [x,y,z,w]
        self.drone_orientation_quaternion = [0.0, 0.0, 0.0, 0.0]
        #initialsing pi   

        # This corresponds to your current orientation of eDrone converted in euler angles form.
        # [r,p,y]
        self.drone_orientation_euler = [0.0, 0.0, 0.0]

        self.drone_orientation_euler_degree=[0.0,0.0,0.0]

        # This is the setpoint that will be received from the drone_command in the range from 1000 to 2000
        # [r_setpoint, p_setpoint, y_setpoint]
        self.setpoint_cmd = [0.0, 0.0, 0.0]

        # The setpoint of orientation in euler angles at which you want to stabilize the drone
        # [r_setpoint, p_psetpoint, y_setpoint]
        self.setpoint_euler = [0.0, 0.0, 0.0]

        # Declaring pwm_cmd of message type prop_speed and initializing values
        # Hint: To see the message structure of prop_speed type the following command in the terminal
        # rosmsg show vitarana_drone/prop_speed

        self.pwm_cmd = prop_speed()
        self.pwm_cmd.prop1 = 0.0
        self.pwm_cmd.prop2 = 0.0
        self.pwm_cmd.prop3 = 0.0
        self.pwm_cmd.prop4 = 0.0

	# derivative variables

        
	self.curr_time=0
	self.prev_time=0
	self.d_time=0

        # initial setting of Kp, Kd and ki for [roll, pitch, yaw]. eg: self.Kp[2] corresponds to Kp value in yaw axis
        # after tuning and computing corresponding PID parameters, change the parameters
        self.Kp = [1.55, 1.55, 5]
        self.Ki = [0, 0, 0]
        self.Kd = [0.677, 0.677, 0]
        # -----------------------Add other required variables for pid here ----------------------------------------------
        #
	self.prev_values = [0,0,0]
	self.max_values = [1024, 1024, 1024, 1024]
	self.min_values = [0, 0, 0, 0]
        #defining error variables
	
	# for storing curretn error
        self.error=[0,0,0]
        # for storing previous error
        self.prev_error=[0,0,0]

	# defining pid for 3 axes 
        self.pid_roll=0
        self.pid_pitch=0
        self.pid_yaw=0
	self.pid_throttle=0
	self.pid_landing=0

	# integral term
        self.iterm_roll=0
	self.iterm_pitch=0
	self.iterm_yaw=0

	#gps term
	#self.gps_error=0
	#self.gps_prev_error=0

        # Hint : Add variables for storing previous errors in each axis, like self.prev_values = [0,0,0] where corresponds to [roll, pitch, yaw]
        #        Add variables for limiting the values like self.max_values = [1024, 1024, 1024, 1024] corresponding to [prop1, prop2, prop3, prop4]
        #                                                   self.min_values = [0, 0, 0, 0] corresponding to [prop1, prop2, prop3, prop4]
        #
        # ----------------------------------------------------------------------------------------------------------

        # # This is the sample time in which you need to run pid. Choose any time which you seem fit. Remember the stimulation step time is 50 ms
        self.sample_time = 16 #0.060  # in seconds

        # Publishing /edrone/pwm, /roll_error, /pitch_error, /yaw_error
        self.pwm_pub = rospy.Publisher('/edrone/pwm', prop_speed, queue_size=1)
        # ------------------------Add other ROS Publishers here-----------------------------------------------------
	self.rollError = rospy.Publisher('/roll_error', ryp_error, queue_size=1)
	self.pitchError = rospy.Publisher('/pitch_error',ryp_error , queue_size=1)
	self.yawError = rospy.Publisher('/yaw_error',ryp_error, queue_size=1)
         
        # -----------------------------------------------------------------------------------------------------------

        # Subscribing to /drone_command, imu/data, /pid_tuning_roll, /pid_tuning_pitch, /pid_tuning_yaw
	rospy.Subscriber('/drone_cmd', edrone_cmd, self.drone_cmd_callback)
        #rospy.Subscriber('/drone_command', edrone_cmd, self.drone_command_callback)
        rospy.Subscriber('/edrone/imu/data', Imu, self.imu_callback)
	rospy.Subscriber('landing',Float64,self.landing_callback)
        #rospy.Subscriber('/pid_tuning_roll', PidTune, self.roll_set_pid)
	#rospy.Subscriber('/pid_tuning_pitch', PidTune, self.pitch_set_pid)
	#rospy.Subscriber('/pid_tuning_yaw', PidTune, self.yaw_set_pid)
	#rospy.Subscriber('/gps_error',Gps_error,self.gps_error)
        
        # -------------------------Add other ROS Subscribers here----------------------------------------------------
        # ------------------------------------------------------------------------------------------------------------

    # Imu callback function
    # The function gets executed each time when imu publishes /edrone/imu/data

    # Note: The imu publishes various kind of data viz angular velocity, linear acceleration, magnetometer reading (if present),
    # but here we are interested in the orientation which can be calculated by a complex algorithm called filtering which is not in the scope of this task,
    # so for your ease, we have the orientation published directly BUT in quaternion format and not in euler angles.
    # We need to convert the quaternion format to euler angles format to understand the orienataion of the edrone in an easy manner.
    # Hint: To know the message structure of sensor_msgs/Imu, execute the following command in the terminal
    # rosmsg show sensor_msgs/Imu


    def landing_callback(self,msg):
        '''
        Purpose:
        ---
        < A callback function to the topic 'landing' >

        Input Arguments:
        ---
        < msg > :  [ <float64> ]
        < to control landing >
    

        Returns:
        ---
        None

        '''
	self.pid_landing=msg.data

    def imu_callback(self, msg):
        '''
        Purpose:
        ---
        < A callback function to the topic '/edrone/imu/data' >
        
        Input Arguments:
        ---
        < msg > :  [ <Imu> ]
        < To get the values of Inertial Measurement Unit >
        
        
        Returns:
        ---
        None
            
        '''
        self.drone_orientation_quaternion[0] = msg.orientation.x
	self.drone_orientation_quaternion[1] = msg.orientation.y
	self.drone_orientation_quaternion[2] = msg.orientation.z
	self.drone_orientation_quaternion[3] = msg.orientation.w

    def drone_cmd_callback(self, msg):
        '''
        Purpose:
        ---
        < A callback function to the topic '/drone_cmd' >
        
        Input Arguments:
        ---
        < msg > :  [ <edrone_cmd> ]
        < To define the roll, pitch and yaw of the drone >
        
        
        Returns:
        ---
        None
            
        '''

        self.setpoint_euler[0] = msg.rcRoll
	self.setpoint_euler[1] = msg.rcPitch
	self.setpoint_euler[2] = msg.rcYaw
	self.pid_throttle=msg.rcThrottle

	

        # --------------------Set the remaining co-ordinates of the drone from msg----------------------------------------------



    # ----------------------------------------------------------------------------------------------------------------------

    def pid(self):
        # -----------------------------Write the PID algorithm here--------------------------------------------------------------

        # Steps:
        #   1. Convert the quaternion format of orientation to euler angles
        #   2. Convert the setpoint that is in the range of 1000 to 2000 into angles with the limit from -10 degree to 10 degree in euler angles
        #   3. Compute error in each axis. eg: error[0] = self.setpoint_euler[0] - self.drone_orientation_euler[0], where error[0] corresponds to error in roll...
        #   4. Compute the error (for proportional), change in error (for derivative) and sum of errors (for integral) in each axis. Refer "Understanding PID.pdf" to understand PID equation.
        #   5. Calculate the pid output required for each axis. For eg: calcuate self.out_roll, self.out_pitch, etc.
        #   6. Use this computed output value in the equations to compute the pwm for each propeller. LOOK OUT FOR SIGN (+ or -). EXPERIMENT AND FIND THE CORRECT SIGN
        #   7. Don't run the pid continously. Run the pid only at the a sample time. self.sampletime defined above is for this purpose. THIS IS VERY IMPORTANT.
        #   8. Limit the output value and the final command value between the maximum(0) and minimum(1024)range before publishing. For eg : if self.pwm_cmd.prop1 > self.max_values[1]:
        #                                                                                                                                      self.pwm_cmd.prop1 = self.max_values[1]
        #   8. Update previous errors.eg: self.prev_error[1] = error[1] where index 1 corresponds to that of pitch (eg)
        #   9. Add error_sum to use for integral component


        #self.curr_time=rospy.get_time()
        #self.d_time=self.curr_time-self.prev_time


        # Converting quaternion to euler angles

        '''
        Purpose:
        ---
        < To find Euler angles from IMU data, and thereby find the speeds of the four propellers >
        
        Input Arguments:
        ---
        None

        Returns:
        ---
        None
        
        '''
        (self.drone_orientation_euler[0], self.drone_orientation_euler[1], self.drone_orientation_euler[2]) = tf.transformations.euler_from_quaternion([self.drone_orientation_quaternion[0], self.drone_orientation_quaternion[1], self.drone_orientation_quaternion[2], self.drone_orientation_quaternion[3]])
         
        self.drone_orientation_euler_degree[1]=(180/PI)*self.drone_orientation_euler[0]
	self.drone_orientation_euler_degree[0]=(180/PI)*self.drone_orientation_euler[1]
	self.drone_orientation_euler_degree[2]=(180/PI)*self.drone_orientation_euler[2]
     
        

        # Convertng the range from 1000 to 2000 in the range of -10 degree to 10 degree for roll axis
        #self.setpoint_euler[0] = self.setpoint_cmd[0] * 0.02 - 30


        # Complete the equations for pitch and yaw axis
        #self.setpoint_euler[1] = self.setpoint_cmd[1] * 0.02 - 30
	#self.setpoint_euler[2] = self.setpoint_cmd[2] * 0.02 - 30
        
        
        # finding the error   
        self.error[0]=self.drone_orientation_euler_degree[0] - self.setpoint_euler[0] 
        self.error[1]=self.drone_orientation_euler_degree[1] - self.setpoint_euler[1]
	self.error[2]=self.drone_orientation_euler_degree[2] - self.setpoint_euler[2]

	#integral for roll
	#self.iterm_roll=(self.iterm_roll + self.error[0] ) * self.Ki[0]
	#self.iterm_pitch=(self.iterm_pitch + self.error[1] ) * self.Ki[1]
	#self.iterm_yaw=(self.iterm_yaw + self.error[2] ) * self.Ki[2]

	# caluclating integral term
	#if(self.error[2]>-10 and self.error[2] < 10):
	#	self.iterm_yaw=(self.iterm_yaw +self.error[2])*self.Ki[2]

        # Definig pid for roll, pitch and yaw

        self.pid_roll=self.error[0]*self.Kp[0] + (self.error[0]-self.prev_error[0] ) * (self.sample_time) *self.Kd[0] + self.iterm_roll
        self.pid_pitch=self.error[1]*self.Kp[1] + (self.error[1]-self.prev_error[1] ) * (self.sample_time) *self.Kd[1] + self.iterm_pitch
	self.pid_yaw=self.error[2]*self.Kp[2] + (self.error[2]-self.prev_error[2] ) * (self.sample_time) *self.Kd[2] + self.iterm_yaw



        
        # Also convert the range of 1000 to 2000 to 0 to 1024 for throttle here itslef
        
        
        self.pwm_cmd.prop1 = 512 + self.pid_roll - self.pid_pitch + self.pid_yaw  - self.pid_throttle + self.pid_landing
        self.pwm_cmd.prop2 = 512 + self.pid_roll + self.pid_pitch - self.pid_yaw  - self.pid_throttle + self.pid_landing
        self.pwm_cmd.prop3 = 512 - self.pid_roll + self.pid_pitch + self.pid_yaw  - self.pid_throttle + self.pid_landing
        self.pwm_cmd.prop4 = 512 - self.pid_roll - self.pid_pitch - self.pid_yaw  - self.pid_throttle + self.pid_landing
 
        
        if self.pwm_cmd.prop1<470:
		self.pwm_cmd.prop1=470
        if self.pwm_cmd.prop1>1023:
		self.pwm_cmd.prop1=1023

        if self.pwm_cmd.prop2<470:
		self.pwm_cmd.prop2=470
        if self.pwm_cmd.prop2>1023:
		self.pwm_cmd.prop2=1023

	if self.pwm_cmd.prop3<470:
		self.pwm_cmd.prop3=470
	if self.pwm_cmd.prop3>1023:
		self.pwm_cmd.prop3=1023

        if self.pwm_cmd.prop4<470:
		self.pwm_cmd.prop4=470
	if self.pwm_cmd.prop4>1023:
		self.pwm_cmd.prop4=1023



        
        # ------------------------------------------------------------------------------------------------------------------------
        
         #self.prev_time=self.curr_time
        (self.prev_error[0],self.prev_error[1],self.prev_error[2])=(self.error[0],self.error[1],self.error[2])
        self.pwm_pub.publish(self.pwm_cmd)
        self.rollError.publish(self.error[0])
        self.pitchError.publish(self.error[1])
        self.yawError.publish(self.error[2])
        #rospy.loginfo("running")
        #print(self.drone_orientation_euler_degree)
       

          
if __name__ == '__main__':

    e_drone = Edrone()
    r = rospy.Rate(e_drone.sample_time)  # specify rate in Hz based upon your desired PID sampling time, i.e. if desired sample time is 33ms specify rate as 30Hz
    r.sleep()
    while not rospy.is_shutdown():
        e_drone.pid()
        r.sleep()
