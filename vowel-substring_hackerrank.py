...
while i < len(S):
    if re.match(
        ...
        m = re.match(
            vowels, S[i + 1 : len(S) - 1]
        )  # Checking consecutive vowel repitions in the string from next index to end-1 pos
        if m and re.match(
            consonants, S[i + len(m.group()) + 1]
        ...
        if m:
            i = (
                i + len(m.group()) + 1
            )  # Skipping the iteration over the length of current pattern by incrementing i with pattern length
        else:
