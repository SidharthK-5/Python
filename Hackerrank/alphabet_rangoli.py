...
def print_rangoli(size):
    ...
    for row in range(size):
        string_index = size - 1
        col_start = chars_in_a_row//2-2*row # In each row, the starting pos will be center - 2*row_number
        ...
        for col in range(col_start, col_end, 2):
            ...
            string_index -= 1
        rangoli[-row-1] = rangoli[row] # Mirror the current row w.r. to the middle row
